...
public sealed partial class CadDocument
{
    ...
    private readonly List<CadBlockDefinition> _blocks = [];
    ...
    public IReadOnlyList<CadBlockDefinition> Blocks => _blocks;
    ...
    public CadBlockDefinition GetBlock(string name) =>
        _blocks.FirstOrDefault(block => string.Equals(block.Name, name, StringComparison.OrdinalIgnoreCase))
        ?? throw new KeyNotFoundException($"Block '{name}' does not exist.");

    public bool TryGetBlock(string name, out CadBlockDefinition? block)
    {
        block = _blocks.FirstOrDefault(candidate => string.Equals(candidate.Name, name, StringComparison.OrdinalIgnoreCase));
        return block is not null;
    }

    public CadEntityProperties GetEntityProperties(Guid id)
    {
        if (_entityProperties.TryGetValue(id, out var properties)) return properties;
        if (_entities.Any(entity => entity.Id == id)) return new CadEntityProperties(_currentLayerName);
    ...
    public void AddRange(IEnumerable<ICadEntity> entities)
    {
        ...
        if (additions.Any(entity => entity is null)) throw new ArgumentException("Entity collection cannot contain null values.", nameof(entities));
        EnsureNoDuplicateIds(additions, excludingIds: []);
        RecordMutation();
        _entities.AddRange(additions);
        foreach (var entity in additions) _entityProperties[entity.Id] = new CadEntityProperties(_currentLayerName);
    ...
    public void AddRange(IEnumerable<(ICadEntity Entity, CadEntityProperties Properties)> entities)
    {
        ...
        var additions = entities.ToArray();
        if (additions.Length == 0) return;
        if (additions.Any(item => item.Entity is null || item.Properties is null)) throw new ArgumentException("Entity/property collection cannot contain null values.", nameof(entities));
    ...
    public bool Replace(Guid id, IEnumerable<ICadEntity> replacements)
    {
        ...
        foreach (var replacement in replacementSnapshot) _entityProperties[replacement.Id] = inherited;
    ...
    public int ReplaceRange(IEnumerable<ICadEntity> replacements)
    {
        ...
        if (snapshot.Any(entity => entity is null)) throw new ArgumentException("Replacement collection cannot contain null values.", nameof(replacements));
        var byId = snapshot.ToDictionary(entity => entity.Id);
        var matched = _entities.Count(entity => byId.ContainsKey(entity.Id));
        if (matched == 0) return 0;
        if (matched != byId.Count) throw new ArgumentException("Every replacement must match an existing entity Id.", nameof(replacements));
        RecordMutation();
        for (var i = 0; i < _entities.Count; i++) if (byId.TryGetValue(_entities[i].Id, out var replacement)) _entities[i] = replacement;
    ...
    public void CreateLayer(CadLayer layer)
    {
        ...
        if (TryGetLayer(layer.Name, out _)) throw new InvalidOperationException($"Layer '{layer.Name}' already exists.");
        RecordMutation();
        _layers.Add(layer);
    ...
    {
        if (string.Equals(name, CadLayer.DefaultLayerName, StringComparison.OrdinalIgnoreCase)) return false;
        ...
        if (string.Equals(_currentLayerName, layer.Name, StringComparison.OrdinalIgnoreCase)) _currentLayerName = CadLayer.DefaultLayerName;
        RaiseChanged(CadDocumentChangeKind.LayerTable);
        return true;
    }
    ...
    {
        if (string.Equals(oldName, CadLayer.DefaultLayerName, StringComparison.OrdinalIgnoreCase)) throw new InvalidOperationException("Layer 0 cannot be renamed.");
        ...
        RecordMutation();
        _layers[_layers.IndexOf(layer)] = layer with { Name = trimmed };
    ...
    public void UpdateLayer(string name, string? colorHex = null, double? lineWeight = null, string? lineType = null, bool? isVisible = null, bool? isLocked = null)
    {
        ...
        var updated = new CadLayer(layer.Name, colorHex ?? layer.ColorHex, lineWeight ?? layer.LineWeight, lineType ?? layer.LineType, isVisible ?? layer.IsVisible, isLocked ?? layer.IsLocked);
    ...
    {
        if (colorHex is not null && !CadLayer.IsValidColor(colorHex)) throw new ArgumentException("Color must use #RRGGBB format or null for ByLayer.", nameof(colorHex));
    ...
    {
        if (lineWeight is not null && (!double.IsFinite(lineWeight.Value) || lineWeight.Value <= 0)) throw new ArgumentOutOfRangeException(nameof(lineWeight));
    ...
    public void DefineBlock(CadBlockDefinition definition, bool replaceExisting = false)
    {
        ArgumentNullException.ThrowIfNull(definition);
        var existing = _blocks.FirstOrDefault(block => string.Equals(block.Name, definition.Name, StringComparison.OrdinalIgnoreCase));
        if (existing is not null && !replaceExisting) throw new InvalidOperationException($"Block '{definition.Name}' already exists.");
        RecordMutation();
        if (existing is not null) _blocks[_blocks.IndexOf(existing)] = definition;
        else _blocks.Add(definition);
        RaiseChanged(CadDocumentChangeKind.BlockTable);
    }

    public bool DeleteBlock(string name)
    {
        var existing = _blocks.FirstOrDefault(block => string.Equals(block.Name, name, StringComparison.OrdinalIgnoreCase));
        if (existing is null) return false;
        if (_entities.OfType<BlockReferenceEntity>().Any(reference => string.Equals(reference.DefinitionName, existing.Name, StringComparison.OrdinalIgnoreCase))) return false;
        RecordMutation();
    ...
    private void Restore(DocumentSnapshot snapshot)
    {
        ...
        _blocks.Clear();
        _blocks.AddRange(snapshot.Blocks);
        _entityProperties.Clear();
    ...
    private void EnsureNoDuplicateIds(IEnumerable<ICadEntity> candidates, IEnumerable<Guid> excludingIds)
    {
        ...
        if (snapshot.Select(entity => entity.Id).Distinct().Count() != snapshot.Length) throw new ArgumentException("CAD entity identities must be unique.", nameof(candidates));
        ...
        if (snapshot.Any(entity => existing.Contains(entity.Id))) throw new InvalidOperationException("A CAD entity with the same identity already exists in the document.");
    }
