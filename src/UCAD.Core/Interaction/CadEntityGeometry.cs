...
public static class CadEntityGeometry
{
    ...
    public static CadRect GetBounds(ICadEntity entity)
    {
        ...
        return entity switch
        {
            ...
            CircleEntity circle => new CadRect(
    ...
    public static double DistanceTo(ICadEntity entity, CadPoint point)
    {
        ...
        return entity switch
        {
            ...
            PolylineEntity polyline => EnumerateSegments(polyline)
    ...
    {
        ArgumentNullException.ThrowIfNull(entity);
        if (!GetBounds(entity).Intersects(rectangle, tolerance))
        {
            return false;
        }
    ...
    public static IReadOnlyList<CadPoint> Intersections(ICadEntity first, ICadEntity second)
    {
        ...
        if (firstSegments.Count > 0 && secondSegments.Count > 0)
        {
            ...
            {
                if (TrySegmentIntersection(a.Start, a.End, b.Start, b.End, out var point))
                {
    ...
    public static IReadOnlyList<CadPoint> GetMidpoints(ICadEntity entity)
    {
        return entity switch
        {
            LineEntity line => [Midpoint(line.Start, line.End)],
            PolylineEntity polyline => EnumerateSegments(polyline)
                .Select(segment => Midpoint(segment.Start, segment.End))
                .ToArray(),
    ...
    public static bool IsPointOnArc(ArcEntity arc, CadPoint point, double radialTolerance = 1e-6)
    {
        ...
        if (radialDistance > radialTolerance)
        {
            return false;
        }
    ...
    private static double DistanceToArc(ArcEntity arc, CadPoint point)
    {
        ...
        if (IsAngleOnArc(arc, angle))
        {
            return Math.Abs((point - arc.Center).Length - arc.Radius);
        }
    ...
    private static bool IsAngleOnArc(ArcEntity arc, double angle)
    {
        ...
        {
            return NormalizePositive(angle - arc.StartAngleRadians) <= arc.SweepAngleRadians + Epsilon;
        }
    ...
    private static CadPoint Midpoint(CadPoint a, CadPoint b) =>
    ...
    private static IEnumerable<(CadPoint Start, CadPoint End)> EnumerateSegments(PolylineEntity polyline)
    {
        ...
        {
            yield return (polyline.Points[i - 1], polyline.Points[i]);
        }
        if (polyline.Closed && polyline.Points.Count > 2)
        {
            yield return (polyline.Points[^1], polyline.Points[0]);
        }
    }

    private static bool SegmentIntersectsRectangle(CadPoint start, CadPoint end, CadRect rectangle, double tolerance)
    {
        if (rectangle.Contains(start, tolerance) || rectangle.Contains(end, tolerance))
        {
            return true;
        }
    ...
    private static bool CircleIntersectsRectangle(CadPoint center, double radius, CadRect rectangle, double tolerance)
    {
        ...
        if (rectangle.Contains(circleBounds, tolerance))
        {
            return true;
        }
        ...
        {
            if (SegmentCircleIntersections(edge.Start, edge.End, center, radius).Count > 0)
            {
                return true;
            }
        }
    ...
    {
        if (rectangle.Contains(arc.StartPoint, tolerance) || rectangle.Contains(arc.EndPoint, tolerance))
        {
            return true;
        }
        foreach (var edge in RectangleEdges(rectangle))
        {
            if (SegmentCircleIntersections(edge.Start, edge.End, arc.Center, arc.Radius)
                .Any(point => IsPointOnArc(arc, point)))
            {
                return true;
            }
        }
    ...
    private static void AddLinearCurveIntersections(
        ICollection<CadPoint> result,
        ...
        foreach (var segment in segments)
        {
            switch (curve)
            {
                case CircleEntity circle:
                    foreach (var point in SegmentCircleIntersections(segment.Start, segment.End, circle.Center, circle.Radius))
                    {
                        AddDistinct(result, point);
                    }
                    break;
                case ArcEntity arc:
                    foreach (var point in SegmentCircleIntersections(segment.Start, segment.End, arc.Center, arc.Radius))
                    {
                        if (IsPointOnArc(arc, point))
                        {
                            AddDistinct(result, point);
                        }
                    }
    ...
    private static IReadOnlyList<CadPoint> SegmentCircleIntersections(
        CadPoint start,
        CadPoint end,
        CadPoint center,
        double radius)
    {
        ...
        foreach (var t in new[] { (-b - root) / (2 * a), (-b + root) / (2 * a) })
        {
            ...
            {
                AddDistinct(points, new CadPoint(start.X + (direction.X * t), start.Y + (direction.Y * t)));
            }
        }
        return points;
    }

    private static void AddCircleCircleIntersections(
        ICollection<CadPoint> result,
        CadPoint firstCenter,
        double firstRadius,
        CadPoint secondCenter,
        double secondRadius)
    {
        var delta = secondCenter - firstCenter;
        var distance = delta.Length;
        if (distance < Epsilon || distance > firstRadius + secondRadius + Epsilon ||
    ...
    {
        if (points.Any(point => (point - candidate).Length <= 1e-7))
        {
            return;
        }
